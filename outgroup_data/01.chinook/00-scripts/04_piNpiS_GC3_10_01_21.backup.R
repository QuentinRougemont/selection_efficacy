...
pnpsgc3=read.table("pnps_gc3.txt",h=T) #new one 
pnpsgc3=read.table("pnps_gc3.1000000.txt",h=T)
##### pNpS ~ median GC3 all pops ###
#pnpsgc3=read.table("../../old/pnps/04.plot/all_pop.pNpS.1mb.txt",h=TRUE)
...
summary(lm(w$Slope ~ w$dist_tot))
pnps <- read.table("../pnps.txt",T)
pnps_slope <- merge(w,pnps, by.x="POP", by.y="myfile")
summary(lm(pnps_slope$Slope ~ pnps_slope$pNpSratio))
